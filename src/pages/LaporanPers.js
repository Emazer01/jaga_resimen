                ...
                <div className='w-100'>
                    ...
                    <div className='p-2 d-flex flex-wrap font-nunito'>
                        ...
                        <div className={`p-1 p-lg-2 pb-3 col-12 d-flex flex-wrap ${lapMasuk[jabatan.tingkat]}`}>
                            <div className='card shadow w-100 col-12'>
                                <h4 className='card-header d-flex border-bottom'>Satuan Bawah
                                ...
                                <div className='card-body p-2 d-flex flex-wrap'>
                                    {subordinates.map(eachSubordinates => {
                                        return (
                                            <div className='p-2 col-12 col-lg'>
                                                <strong className='border-bottom border-dark border-3 fs-5'>{eachSubordinates.subordinates_nama}</strong>
                                                <table className='m-0'>
                                                    <tbody>
                                                        {eachSubordinates.pejabat_menkorps.map(x => {
                                                            return (
                                                                <>
                                                                    <tr><td className='p-0 border-none'><small className='fw-bold'>{x.jab_nama} :</small></td></tr>
                                                                    <tr><td className='p-0 border-none'><small>{x.kadet_nama}</small></td></tr>
                                                                </>
                                                            )
                                                        })}
                                                        {eachSubordinates.pejabat_dinas.map(x => {
                                                            return (
                                                                <>
                                                                    <tr><td className='p-0 border-none'><small className='fw-bold'>{x.jab_nama} :</small></td></tr>
                                                                    <tr><td className='p-0 border-none'><small>{x.kadet_nama}</small></td></tr>
                        ...
                        <div className={`p-1 p-lg-2 pb-3 col-12 d-flex flex-wrap ${lapMasuk[jabatan.tingkat]}`}>
                            <div className='card shadow w-100 col-12'>
                                <h4 className='card-header d-flex border-bottom'>Laporan Hari Ini
                                    <i className="fs-2 ms-auto bi bi-file-earmark-person-fill"></i>
                                </h4>
                                <div className='card-body p-2 d-flex flex-wrap'>
                                    <div className='p-2 col-12 col-lg-6'>
                                        {subordinates.map(eachSubordinates => {
                                            return (
                                                <div key={eachSubordinates.subordinates_id}>
                                                    {eachSubordinates.lap_apel.map(lap_apel => {
                                                        ...
                                                        if (lap_apel.tanpa_keterangan != null) {
                                                            warning = ''
                                                        }
                                                            ...
                                                            return (
                                                                <div key={lap_apel.apel_id} className='border-5 border-dark p-1 ps-3 border-start position-relative'>
                                                                    <button className={`btn btn-primary rounded-0 w-100 text-start`} data-bs-toggle="collapse" data-bs-target={`#detail-${lap_apel.apel_id}`} aria-expanded="true">
                                                                        <strong>{lap_apel.jenis_apel_nama} {lap_apel.satuan}</strong><br />
                                                                        <small>{tanggal}</small>
                                                                        <small> | {lap_apel.pangkat_singkat} {lap_apel.kadet_nama}</small>
                                                                    </button>
                                                                    <table className='table collapse border' id={`detail-${lap_apel.apel_id}`}>
                                                                        <tbody>
                                                                            ...
                                                                            <tr>
                                                                                <td colSpan='4' className='p-0'><a href={`/laporan/pers/apel?tingkat=${lap_apel.tingkat}&nomor=${lap_apel.apel_id}`} className='btn w-100 text-end rounded-0'>Selengkapnya <i className="bi bi-box-arrow-up-right"></i></a></td>
                                                                            </tr>
                                                                        </tbody>
                                                                    </table>
                                                                    <span className='rounded-circle border-4 border-dark timeline-circle border position-absolute'></span>

                                                                    <i id={`lap-warning-${lap_apel.apel_id}`} class={`bi bi-exclamation-circle-fill position-absolute lap-warning fs-4 text-warning ${warning}`}></i>
                                                                </div>
                                                            )
                                                        }
                                                    })
                                        ...
                                        <span className='border-4 border-dark border position-absolute' style={{ width: 15, left: 11 }}></span>
                                    </div>
                                    <div className='p-2 col-12 col-lg-6'>
                                        <strong>Belum Kirim Laporan Apel :</strong>
                                        {subordinates.map(x => {
                                            if (x.lap_apel.length == 0) {
                                                return (
                                                    <div>{x.subordinates_nama} (2 laporan)</div>
                                                )
                                            }
                                            else if (x.lap_apel.length == 1) {
                                                return (
                                                    <div>{x.subordinates_nama} (1 laporan)</div>
                                                )
                                            }
                                        })}
                                    </div>
                                        ...
                                        <div className='p-2 col-12 col-lg-6'>
                                            ...
                                            <div className='mt-2 position-relative'>
                                                {lapApel.map(lapApel => {
                                                    ...
                                                    if (lapApel.tanpa_keterangan != null) {

                                                    }
                                                    return (
                                                        <div key={lapApel.apel_id} className='border-5 border-dark p-1 ps-3 border-start position-relative'>
                                                            <button className={`btn ${editable['button-type'][lapApel.editable]} rounded-0 w-100 text-start d-flex`} data-bs-toggle="collapse" data-bs-target={`#detail-my-${lapApel.apel_id}`} aria-expanded="true">
                                                            ...
                                                            <table className='table collapse border' id={`detail-my-${lapApel.apel_id}`}>
                                                                <tbody>
                                                                    <tr>
                                                                        <th>Jumlah</th>
